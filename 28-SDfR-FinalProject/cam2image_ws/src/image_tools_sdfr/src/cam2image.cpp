...
private:
  ...
  void timerCallback()
  {
    ...
    if (remote_mode_) {
      //frame = remote_cap.get_frame(width_, height_);
      frame = remote_cap.get_frame();
  ...
  bool help(const std::vector<std::string> & args)
  {
      ...
      std::find(args.begin(), args.end(), "-h") != args.end())
    {
      ...
      ss << std::endl;
      ss << "  remote_mode\t\tGet images from a remote camera. Either 'true' or 'false' (default)";
      ss << std::endl;
      ss << "  remote_timeout\tRemote timeout before the node shuts down. Default value is 7";
      ss << std::endl;
      ss << "  socket_ip\t\tIP to connect to remote camera. Default value is 127.0.0.1";
      ss << std::endl;
      ss << "  socket_port\t\tPort to connect to remote camera. Default value is 80";
      ss << std::endl;
  ...
  void parse_parameters()
  {
    ...
    // Declare and get parameters related to remote mode.
    ...
    rcl_interfaces::msg::ParameterDescriptor remote_timeout_desc;
    remote_timeout_desc.description = "The amount of time in seconds before the node automatically shuts down in remote mode";
    remote_timeout_ = this->declare_parameter("remote_timeout", 7, remote_timeout_desc);
